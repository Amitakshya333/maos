 ...
 */
export function createBranchFrom(branchName: string, baseBranch: string, cwd: string): void {
  if (branchExists(branchName, cwd)) {
    git(['checkout', branchName], cwd);
  } else {
...
export function getBranchList(cwd: string): GitBranchInfo[] {
  const raw = git(['branch', '--format=%(refname:short)|%(HEAD)|%(objectname:short)|%(subject)'], cwd);
...
export function stashChanges(cwd: string, message?: string): boolean {
  ...
  const msg = message || 'MAOS auto-stash';
  gitSafe(['stash', 'save', msg], cwd);
...
export function getDiffSummary(cwd: string): GitDiffSummary {
  const files = git(['diff', '--name-only', 'HEAD'], cwd).split('\n').filter(Boolean);
 ...
 */
export function getBranchDiff(targetBranch: string, baseBranch: string, cwd: string): string {
 ...
 */
export function getBranchPatch(targetBranch: string, baseBranch: string, cwd: string): string {
...
export function getRecentCommits(
  ...
  const raw = gitSafe(['log', `-${count}`, '--pretty=format:%h|%s|%ci'], cwd);
 ...
 */
export function prepareAgentBranch(agentId: string, taskId: string, baseBranch: string, cwd: string): string {
 ...
 */
export function tagAgentWork(branchName: string, cwd: string): void {
